...
for (const line of lines) {
  if (line.includes('export async function parseExcelMasterPlan')) capture = true;
  if (line.includes('export function isMasterPlanFormat')) capture = false;
  if (capture) funcLines.push(line);
}
let funcStr = funcLines.join('\n');
funcStr = funcStr.replace('export async function parseExcelMasterPlan', 'async function parseExcelMasterPlan');
...
fs.writeFileSync('final_run.cjs', pre + funcStr + post);
