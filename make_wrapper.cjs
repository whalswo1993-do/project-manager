...
let funcCode = code.substring(start, end).replace('export async function parseExcelMasterPlan', 'async function parseExcelMasterPlan');
...
parseExcelMasterPlan(csv).then(res => {
  res.forEach(p => {
    ...
    for(let k in p._deptMap) {
      console.log('  ' + k + ': ' + p._deptMap[k].total);
    }
  });
}).catch(e => console.error(e));
`;

fs.writeFileSync('test_run_4.cjs', wrapper);
