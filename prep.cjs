...
// Paste parseExcelMasterPlan here...
const fs = require('fs');
...
funcCode = funcCode.replace(/if\(deptName&&\(/, 'console.log(`[${r}] Adding ${deptName} (${rowTotal}) to ${currentProject.equipment}`); if(deptName&&(');
funcCode = funcCode.replace(/currentProject\._totalMandayVal=rowTotal;/, 'console.log(`[${r}] SET TOTAL MANDAY ${rowTotal} to ${currentProject.equipment}`); currentProject._totalMandayVal=rowTotal;');

fs.writeFileSync('temp_func.cjs', funcCode + '\nmodule.exports={parseExcelMasterPlan};');
