...
const rawHtml = fs.readFileSync('scripts/full_width_html.html', 'utf8');
const text = fs.readFileSync('scripts/full_width_text.txt', 'utf8');

const startIdx = rawHtml.indexOf('<html');
const html = startIdx !== -1 ? rawHtml.slice(startIdx) : rawHtml;

console.log('--- 1. Testing Full Width HTML ---');
try {
