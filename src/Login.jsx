...
export default function Login(){
 ...
 async function submit(e){e.preventDefault(); const normalized=email.trim().toLowerCase();
  if(!normalized.endsWith(DOMAIN)) return setMessage(`회사 이메일(${DOMAIN})만 사용할 수 있습니다.`);
  ...
  setBusy(false); if(result.error) setMessage(result.error.message); else if(signup) setMessage("인증 메일을 보냈습니다. 메일의 확인 링크를 눌러주세요.");
 }
 return <main className="login-page"><form className="login-card" onSubmit={submit}>
  <b className="brand">TEAM PROJECT HUB</b><h1>{signup?"회사 계정 가입":"팀원 로그인"}</h1>
  ...
  <button type="button" className="link" onClick={()=>{setSignup(!signup);setMessage("")}}>{signup?"이미 계정이 있나요? 로그인":"처음인가요? 회원가입"}</button>
