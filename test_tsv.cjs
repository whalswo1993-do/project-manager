...
// Inject logging
funcCode = funcCode.replace(/if\(deptName&&\(/, 'fs.appendFileSync("log.txt", `[${r}] Adding ${deptName} (${rowTotal}) to ${currentProject.equipment}\\n`); if(deptName&&(');

fs.writeFileSync('temp_parser.cjs', funcCode + '\nmodule.exports={parseExcelMasterPlan};');

const { parseExcelMasterPlan } = require('./temp_parser.cjs');
const xlsx = require('xlsx');
const files = fs.readdirSync('.').filter(f => f.endsWith('.xlsx'));
const file = files.find(f => f.includes('v5'));
const wb = xlsx.readFile(file);

// Generate TSV string to mimic user paste
...
parseExcelMasterPlan(csv).then(res => {
  res.forEach(p => {
    ...
    fs.appendFileSync('log.txt', p.equipment + ': ' + total + '\n');
  });
});
